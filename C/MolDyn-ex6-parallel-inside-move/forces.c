 ...
 */
  ...
  void forces(int npart, double x[], double f[], double side, double rcoff){
    ...
    #pragma omp barrier
//#pragma omp parallel for reduction(+:epot) reduction(-:vir) shared(rcoffs,sideh,x,f) num_threads(8) schedule(runtime)
#pragma omp for reduction(+:epot) reduction(-:vir) schedule(runtime) 
